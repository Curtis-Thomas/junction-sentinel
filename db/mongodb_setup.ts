...
async function setupDatabase() {
  ...
  try {
    ...
    if (droneCount === 0 || process.argv.includes('--force')) {
      ...
      console.log(`Skipped seeding drones: ${droneCount} records already exist`);
    }
    ...
    await dronesCollection.createIndex({ "location.latitude": 1, "location.longitude": 1 });
    ...
    if (userCount === 0) {
      ...
      console.log(`👥 Skipped seeding users: ${userCount} records already exist`);
    }
    ...
    if (logCount === 0) {
      ...
      console.log(` Skipped seeding audit logs: ${logCount} records already exist`);
    }
    ...
    console.log(`   auditLogs  : ${await auditLogsCollection.countDocuments()}`);

    // === SAMPLE QUERIES ===
    console.log('\n🔍 Sample Queries That Should Work Now:');

    const activeDrones = await dronesCollection.countDocuments({ status: 'Active' });
